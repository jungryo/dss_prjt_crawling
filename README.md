...
웹페이지에서 사용자의 출발지점 2곳을 입력하고 음식 종류를 선택하면 이동경로의 1/3지점과 2/3지점을 기준으로 반경 500m 정도 내에 있는 음식점을  
DB에서 select한 후 별점을 기준으로 5개 정도의 음식점만 추천해주는 서비스를 제공하고 싶었으나 프론트엔드 부분과 백엔드를 연결하는 부분에서 공부가 미흡했다.  
...
또한, MongoDB와 javascript를 연결하는 어댑터가 필요하다는 것을 늦게 알게 되어 프로젝트 기간에 맞추지 못하게 되었다. 
