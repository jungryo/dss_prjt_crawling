...
각 사이트 별로 크롤링한 데이터 중복값 제거 > 5개의 데이터(각 사이트 별 데이터) merge > 음식점 종류에 따라 카테고리화 > 각 사이트별 별점을 5점을 기준으로 환산, null값은 각 사이트의 최소값으로 채움 > 각 사이트의 매출액을 기준으로 별점에 가중치를 두어 자체 별점 생성   **별점 전처리 내용**
```
# 네이버 점수 변환
